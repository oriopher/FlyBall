...
class Tello:
    ...
    def __init__(self,
        ...
        self.drones[host] = {'responses': [], 'state': {}}
    ...
    def get_own_udp_object(self):
        ...
        host = self.address[0]
        return self.drones[host]

    @staticmethod
    def udp_response_receiver(tello):
        ...
        while True:
            try:
                data, address = tello.client_socket.recvfrom(1024)

                address = address[0]
                Tello.LOGGER.debug('Data received from {} at client_socket'.format(address))

                if address not in tello.drones:
                    continue

                tello.drones[address]['responses'].append(data)
    ...
    def udp_state_receiver(tello):
        ...
        state_socket.bind((tello.iface_ip, Tello.STATE_UDP_PORT))
        ...
        while True:
            try:
                ...
                tello.drones[address]['state'] = Tello.parse_state(data)
    ...
    def end(self):
        ...
        host = self.address[0]
        if host in self.drones:
            del self.drones[host]
