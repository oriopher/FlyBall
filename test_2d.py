...
def interactive_loop(image_3d: Image3D, colors: ColorBounds, borders: Borders, loop_status: Status, left_cam: Camera,
                     tello_1: Tello, tello_2: Tello) -> bool:
...
def capture_video(tello_1: Tello, tello_2: Tello, cameras_distance, left: Camera, right: Camera, colors: ColorBounds,
                  borders: Borders, method='parallel'):
    ...
    while (True):
        ...
        if frame_counter > len(old_images_vel):
            ...
            balloon_exist, drone_1_exist, drone_2_exist = image_now.calculate_all_distances(left, right,
                                                                                            cameras_distance,
                                                                                            method=method)
            if not balloon_exist:
                image_now.phys_x_balloon, image_now.phys_y_balloon, image_now.phys_z_balloon = image_old.get_phys_balloon(
                    0), image_old.get_phys_balloon(1), image_old.get_phys_balloon(2)
            if not drone_1_exist:
                image_now.phys_x_drone_1, image_now.phys_y_drone_1, image_now.phys_z_drone_1 = image_old.get_phys_drone_1(
                    0), image_old.get_phys_drone_1(1), image_old.get_phys_drone_1(2)
            if not drone_2_exist:
                image_now.phys_x_drone_2, image_now.phys_y_drone_2, image_now.phys_z_drone_2 = image_old.get_phys_drone_2(
                    0), image_old.get_phys_drone_2(1), image_old.get_phys_drone_2(2)

            image_now.calculate_mean_velocities(old_images_vel)

        text_balloon_coor = "c(%.0f,%.0f,%.0f)" % (
        image_now.phys_x_balloon, image_now.phys_y_balloon, image_now.phys_z_balloon)
        text_drone_1_coor = "c(%.0f,%.0f,%.0f)" % (
        image_now.phys_x_drone_1, image_now.phys_y_drone_1, image_now.phys_z_drone_1)
        text_drone_2_coor = "c(%.0f,%.0f,%.0f)" % (
        image_now.phys_x_drone_2, image_now.phys_y_drone_2, image_now.phys_z_drone_2)
        ...
        draw_xy_display(borders, image_now.phys_x_balloon, image_now.phys_y_balloon, image_now.phys_x_drone_1,
                        image_now.phys_y_drone_2, loop_status.dest_coords[0], loop_status.dest_coords[1])

        state.run(**{'recognizable_object': image_now, 'loop_status': loop_status, 'tello': tello, 'borders': borders})
        transition = state.to_transition(**{'recognizable_object': image_now, 'loop_status': loop_status, 'tello': tello})
        if transition:
...
if __name__ == "__main__":
    ...
    distance = 58
    while continue_test:
        continue_test, colors = capture_video(tello_1, tello_2, distance, left, right, colors, borders,
                                              method='parallel')
