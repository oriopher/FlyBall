...
class RecognizableObject:
    ...
    def detect_and_set_coordinates(self, left: Camera, right: Camera, d, frame_counter):
        self.time = datetime.datetime.now()
        self.detect_pixel_coordinates()
        self.object_exists = False
        if self.frame_left.x != 0 and self.frame_right.x != 0:
            self.object_exists = True
            self._calculate_coordinates(left, right, d, frame_counter)
    ...
    def _calculate_coordinates(self, left: Camera, right: Camera, d, frame_counter):
