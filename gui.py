from common import display_frames
...
class Gui:
    ...
    distance_col = sg.Column([[sg.Text('Distance:'), sg.InputText(size=(4, 1), key = 'distance'), sg.Button('recieved_input', visible=False, bind_return_key=True)],
                            [sg.Text('Home 1 = ', key='home_1'), sg.InputText(size=(8, 1), key='set_home_1')],
                            [sg.Text('Home 2 = ', key='home_2'), sg.InputText(size=(8, 1), key='set_home_2')]], element_justification='c')

    play_func = sg.Column([[sg.Button('Take Off'), sg.Button('Go Home'), sg.Button('Start Play'), sg.Button('Quit')]], element_justification='c')

    left_camera_col = sg.Column([[sg.Text('Left Camera:'), sg.Push()],
                            [sg.Text('Index:'), sg.InputText(size=(4, 1), key='left_index'), sg.Checkbox('Flip', key='flip_left')]], element_justification='c')

    right_camera_col = sg.Column([[sg.Text('Right Camera:'), sg.Push()],
                            [sg.Text('Index:'), sg.InputText(size=(4, 1), key='right_index'), sg.Checkbox('Flip', key='flip_right')]], element_justification='c')

    color_bounds_col1 = sg.Column([[sg.Text('THRESHOLD', font=("Helvetica", 8)), sg.Slider((1, 600), 120, orientation='h',key='SLIDER_T', enable_events=True, size=(11, 11))],
                        [sg.Text('H RANGE     ', font=("Helvetica", 8)), sg.Slider((0, 30), 15, orientation='h',key='SLIDER_H', enable_events=True, size=(11, 11))]], element_justification='c')
    ...
    layout = [ [sg.Push(), sg.Frame('Cameras', [[left_camera_col, right_camera_col]]), sg.Frame('Color Bounds', [[color_bounds_col1, color_bounds_col2]]), sg.Push()],
                [sg.Push(), sg.Frame('Colors and Borders', [[colors_col, read_col]]), sg.Frame('', [[distance_col]]), sg.Push()],
                [sg.Push(), left_img_col, right_img_xy_col, sg.Push()],
                [sg.Push(), sg.Frame('Play', [[play_func]]), sg.Frame('State 1', [[sg.Text('', key='state_1')]]), sg.Frame('State 2', [[sg.Text('', key='state_2')]]), sg.Push()]]
